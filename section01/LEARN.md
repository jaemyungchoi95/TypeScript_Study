...
<ul>
  ...
  <li>
    moduleDetection : 모듈감지를 하는 모드를 설정해준다. force로 설정시 타입스트립트가 자동으로 모듈시스템을 사용하는 코드로 추가해준다고 함.
  </li>
</ul>
<br/>

![alt text](image-14.png)
분명 다른 파일에 선언했음에도 블록 범위 변수 문제가 생기는 이유는 타입스크립트는 기본적으로 모든 ts 파일을 전역 모듈로 보기 때문이다.
그렇다고 해서 변수를 a1 이런식으로 선언하면 파일을 나눠놓은 이유가 없기 때문에 아래와 같은 해결방법이 필요하다.

1. export{} , import 같은 명령어를 파일에 한번이라도 작성하면 그 파일은 독립된 파일로 인식된다.
2. tsconfig.json 파일에 옵션 하나를 추가한다.
